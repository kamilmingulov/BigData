...
//    conf.setMaster("local[2]")
    ...
    val tra = sparkSession.sql("select InvoiceNo,StockCode from transactTable where StockCode is not null and InvoiceNo is not null")
    val dic = sparkSession.sql("select StockCode, Description from transactTable " +
      "where StockCode is not null and Description is not null").distinct().rdd.map(x => (x(0).toString, x(1).toString)).collect().toMap
